...
const REPORT_PATH = 'docs/verification/unit-coverage.json';

try {
    // 1. 运行 Go 测试并生成 profile
    // 注意：如果 src/cp-core 下尚无有效 go 文件或包，此步可能会报错
    execSync('go test -cover ./src/... -coverprofile=coverage.out || true');
    ...
    // 生产环境应使用 go tool cover -func=coverage.out
