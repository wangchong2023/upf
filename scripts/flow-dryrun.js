/**
 * @职责: IPD 全流程自愈演练编排器 v3.1 (Semantic Precision Edition)
 * @作者: Gemini CLI
 */
...
const STAGE_METADATA = {
    'TR1': { role: 'PRODUCT/PM/SE', label: 'SRS 需求集' },
    'TR2': { role: 'SE', label: 'RTM 跟踪矩阵' },
    'TR3': { role: 'ARCHITECT', label: 'SDS 设计文档' },
    'TR4': { role: 'DEV', label: '生产源码' },
    'TR5': { role: 'TESTER', label: 'RAT 验收结果' },
    'TR6': { role: 'PRODUCT/QA/PM', label: '版本归档包' }
};
...
function runStage(stage) {
    ...
    while (retryCount <= 1) {
        try {
            execSync('node scripts/mgr-agent-orchestrator.js', { 
                stdio: 'pipe', env: { ...process.env, IPD_DRYRUN: 'true' }
            });
            const details = getDeliverableDetails();
            let summary = "";
            if (stage === 'TR1') summary = details.requirements[0] || "Init SRS";
            else if (stage === 'TR2') summary = `${details.irs.length} IRs (初始需求) / ${details.srs.length} SRs / ${details.specs.length} ARs`;
            else if (stage === 'TR3') summary = "SDS Architecture & Subsystem Specification";
            else if (stage === 'TR4') summary = `${details.codeFiles.length} Source Files (${details.codeFiles[0].split(' ')[0]})`;
            else if (stage === 'TR5') summary = `${details.ratScenarios.length} Acceptance Tests Completed`;
            else if (stage === 'TR6') summary = "Release Quality Gate & SBOM Archiving";

            const statusLabel = retryCount > 0 ? '🩹 Healed' : '✅ Passed';
 ...
 */
async function main() {
    const originalMilestone = fs.existsSync(MILESTONE_FILE) ? fs.readFileSync(MILESTONE_FILE, 'utf-8') : 'GENERAL';
    try {
        console.log(`\n🚀 Starting Expert IPD Dry-run (v3.1) - ${new Date().toLocaleString()}`);
        printTableHeader();
        ...
        console.log('='.repeat(170));
        let report = "# IPD 全生命周期全链路语义化交付报告 (v3.1)\n\n";
