...
try {
    ...
    const rules = {
        ...
        "STRICT_EQUALITY": {
            check: (content) => !content.match(/\s==\s|\s!=\s/),
            fix: (content) => content.replace(/\s==\s/g, ' === ').replace(/\s!=\s/g, ' !== '),
            message: "Found unsafe comparison (== or !=). Use === or !== instead."
        },
        "NO_VAR": {
            check: (content) => !content.match(/\bvar\s/),
            fix: (content) => content.replace(/\bvar\s/g, 'let '),
            message: "Use of 'var' detected. Use 'const' or 'let' instead."
        },
        ...
        "NO_HARDCODED_PATHS": {
            check: (content, filename) => {
                // 核心配置文件和审计工具自身允许包含路径字面量作为定义或参考
                if (filename === 'mgr-config.js' || filename === 'mgr-script-audit.js' || filename.endsWith('.sh')) return true;
                
                // 检查是否包含 docs/ 或 src/ 字面量
                const pathLiteralRegex = /['"](docs\/|src\/)/;
                return !pathLiteralRegex.test(content);
            },
            message: "Hardcoded physical path detected (e.g., 'docs/' or 'src/'). Please use config.PATHS instead."
        }
    };
