...
try {
    ...
    execSync('node scripts/core/mgr-role-gate.js --action=CHARTER_LOCK', { stdio: 'inherit' });
} catch (e) {
...
try {
    ...
    // 在实际流程中，这可能涉及生成或校验 config.PATHS.CHARTER
...
try {
    ...
    execSync('node scripts/core/mgr-role-gate.js --action=DCP_PASS', { stdio: 'inherit' });
    // 调用决策处理器处理 DCP 相关的逻辑
...
try {
    ...
    execSync('node scripts/core/mgr-role-gate.js --action=GTM_AUDIT', { stdio: 'inherit' });
    // 检查产品规格书 config.PATHS.PRODUCT_SPEC 是否存在且完备
